...
import com.net.jpcap.capture.Analyse;
import com.net.jpcap.capture.Captor;
import com.net.jpcap.entity.PacketInfo;
					...
					public void run(){
						...
						while(flag){
							...
							if(packets.size()>count){
								...
								info = analyse.getDetail(packet);
								if(info.getType().name().equalsIgnoreCase(protocol)){
									System.out.println("capture success!");
									result.add(packet);
									Vector row = new Vector();
									row.add(++num);
									row.add(info.getMsgTime());
									row.add(info.getLength());
									row.add(info.getSrcMac());
									row.add(info.getDestMac());
									row.add(info.getType());
									row.add(info.getSrcIp());
									row.add(info.getDescIp());
